...
function defaultHandsPlans() {
  return [
    ...
    { id:"han-24", name:"확장(24국) - E,S,W,N + E,S", sequence:[
      "E1","E2","E3","E4","S1","S2","S3","S4","W1","W2","W3","W4","N1","N2","N3","N4","E1","E2","E3","E4","S1","S2","S3","S4"
    ]}
  ];
}
...
function readJson(key,fallback){ try{ const raw=localStorage.getItem(key); if(!raw) return fallback; return JSON.parse(raw);}catch{ return fallback; } }
...
function clampInt(v,min,max){ const x=Number(v); if(Number.isNaN(x)) return min; const t=Math.trunc(x); return Math.max(min, Math.min(max, t)); }
function clampMaybeInt(v,min,max){ const x=Number(v); if(Number.isNaN(x)) return null; const t=Math.trunc(x); return Math.max(min, Math.min(max, t)); }
...
function saveSnapshot(){
  const snap = JSON.parse(JSON.stringify({ ruleSet, runtime: stripHistory(runtime), handsPlans, activeRuleSetId }));
...
function openModal(title, bodyHtml, onOk){
  ...
  els.modalOk.onclick=(e)=>{ e.preventDefault(); const ok=onOk?.(); if(ok!==false) els.modal.close("ok"); };
  els.modal.showModal();
}
...
function afterWin(winner){
  clearRiichiFlags();
  if(isDealer(winner) && ruleSet.renchan.onWin) runtime.roundState.honba += 1;
...
function afterDraw(tenpais){
  ...
  if(!(dealerTenpai && ruleSet.renchan.onTenpai)){ dealerAdvance(); handAdvance(); }
...
function applyAutoScaleForMobileLandscape(){
  ...
  const topH = topbar && !topbar.classList.contains("hidden")
...
els.resetBtn.addEventListener("click", ()=>{
  const opts = runtime.players.map((p,i)=>`<option value="${i}">${escapeHtml(p.name)} (현재 자리 ${i})</option>`).join("");
  openModal("리셋 (동 위치 선택)",`
    <p class="small">리셋 후 “동(East)”이 될 사람을 선택해줘.</p>
    ...
    <div class="grid2">
      ...
      <div class="card">
        ...
        <div class="row">
          ...
          <button class="btn" id="clearWinnerInputsBtn" type="button">개별 비우기</button>
        </div>
...
function wireMultiRonUI(){
  ...
  if(copyBtn) copyBtn.onclick=()=>{
    const fu=document.getElementById("fu_common")?.value ?? "";
    const han=document.getElementById("han_common")?.value ?? "";
    for(const i of getChecked()){
      const fuI=document.getElementById(`fu_w${i}`);
      const hanI=document.getElementById(`han_w${i}`);
      if(fuI) fuI.value=fu;
      if(hanI) hanI.value=han;
    }
    renderPreview();
  };
...
function openDrawModal(){
  ...
  openModal("유국 (텐파이 정산)",`
    <p class="small">표준: 텐파이 인원끼리 3000점 정산. 유국이면 본장 +1.</p>
...
function openSettingsModal(){
  const presetOptions=ruleSets.map(r=>`<option value="${r.id}" ${r.id===ruleSet.id?"selected":""}>${escapeHtml(r.name)}</option>`).join("");
  const hpOptions=handsPlans.map(h=>`<option value="${h.id}" ${h.id===runtime.roundState.handsPlanId?"selected":""}>${escapeHtml(h.name)}</option>`).join("");
  ...
  openModal("⚙️ 설정",`
    ...
    <div class="card">
      ...
      <div class="field"><label>멀티론</label>
      ...
      <div class="field"><label>공탁 유국시</label>
        <select id="potCarry">
    ...
    document.getElementById("settleTable").innerHTML = `
      <table>
        <thead>
          <tr>
            <th>순위</th><th>플레이어</th>
            <th class="right">점수</th><th class="right">점수-리턴</th>
            <th class="right">오카</th><th class="right">우마</th>
